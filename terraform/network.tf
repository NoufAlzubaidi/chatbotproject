...
# VMSS Subnet Resources
resource "azurerm_subnet" "vmss_subnet" {
  name                 = "${azurerm_virtual_network.vnet.name}-${var.vmss_subnet_name}"
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.vnet.name
...
resource "azurerm_subnet_network_security_group_association" "vmss_subnet_nsg_associate" {
  depends_on                = [azurerm_network_security_rule.vmss_nsg_rule_inbound]
...
resource "azurerm_network_security_rule" "vmss_nsg_rule_inbound" {
  ...
  name                        = "VMSS-Rule-Port-${each.value}"
  priority                    = each.key
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
